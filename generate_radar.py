...
def fetch_stats(username, token):
    ...
    if not resp.ok:
        raise RuntimeError(
            f"HTTP {resp.status_code} from GitHub API: {resp.text}"
        )
    ...
    if "data" not in body:
        raise RuntimeError(f"Unexpected API response (no 'data' key): {body}")

    user = body["data"].get("user")
    if user is None:
        raise RuntimeError(f"User '{username}' not found or token lacks read:user scope")
...
def generate_svg(stats, path):
    ...
    for lvl in [0.25, 0.5, 0.75, 1.0]:
        pts = " ".join(
            f"{x:.1f},{y:.1f}" for x, y in [pt(a, r * lvl) for a in angles]
        )
    ...
    dots = "".join(
        ...
        f'stroke="#2d6a2d" stroke-width="2"/>'
        for x, y in data_pts
    )
...
if __name__ == "__main__":
    ...
    print("Fetching contribution stats for Pollycarp...")
